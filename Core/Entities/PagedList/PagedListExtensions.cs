...
namespace Core.Entities.PagedList
{
    public static class PagedListExtensions
    {
        ...
        public static IPagedList<TResult> ToPagedList<TEntity, TResult>(this IQueryable<TEntity> source, int pageSize, int pageNumber = 0, Expression<Func<TEntity, TResult>> selector = null)
        {
            int count = source.Count();
            int totalPages = (int)Math.Ceiling((double)count / pageSize);
            if (totalPages == 0) totalPages = 1;
